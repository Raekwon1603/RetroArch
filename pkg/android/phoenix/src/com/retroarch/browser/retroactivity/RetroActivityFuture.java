...
public final class RetroActivityFuture extends RetroActivityCamera {
  ...
  @Override
  public void onStop() {
    // See docs/retroarch-fork-notes.md in the super_metroid-android
    // project. bsnes-hd beta only writes its save file from
    // retro_unload_game(), which RetroArch only calls on a clean content
    // unload (backing out to the menu, or quitting properly). It never
    // gets called on a task swipe away, backgrounding, or force close, so
    // in game saves were silently lost every time the app got closed
    // normally. onStop() is the real moment that happens, so force a save
    // here no matter how the loaded core would otherwise have handled it.
    // Harmless no-op if there's no core loaded yet or it's not the
    // patched build, nativeForceSaveGame returns false either way, same
    // as for the other cores this fork can still load.
    nativeForceSaveGame();
